...
public final class RazvanTeleOp extends BaseOpMode
{
	...
	private final static class Bindings
	{
		...
		private final static class Arm
		{
			...
			private static final InputSystem.Key SKIP_ARM = new InputSystem.Key("b");
	...
	protected void OnInitialize()
	{
		...
		mediaRecorderPipeline = new MediaRecorderPipeline(MakeVideoFile("razvan"));
	...
	private void Arm()
	{
		if (!armBusy && armInput.wasPressedThisFrame(Bindings.Arm.SKIP_ARM)) // go back to IDLE phase
		{
	...
	private volatile boolean droneLaunched = false;
